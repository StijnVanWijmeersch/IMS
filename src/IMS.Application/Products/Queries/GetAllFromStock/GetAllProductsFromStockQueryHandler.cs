...
internal sealed class GetAllProductsFromStockQueryHandler : IRequestHandler<GetAllProductsFromStockQuery, Result<Page<ProductDto>>>
{
    private readonly IIMSDbContext _context;
    private readonly ICacheService _cacheService;

    public GetAllProductsFromStockQueryHandler(IIMSDbContext context, ICacheService cacheService)
    {
        _context = context;
        _cacheService = cacheService;
    }

    public async Task<Result<Page<ProductDto>>> Handle(GetAllProductsFromStockQuery request, CancellationToken cancellationToken)
    {
        Page<ProductDto>? cacheResult = await _cacheService
           .GetAsync<Page<ProductDto>>($"products-stock-{request.Cursor}-{request.PageSize}", cancellationToken);
        ...
        var products = _context.Products
            .AsNoTracking()
            .OrderBy(product => product.Id)
            .Where(product => product.Id >= (ulong)request.Cursor && product.InStock)
        ...
        await _cacheService
                .SetAsync($"products-stock-{request.Cursor}-{request.PageSize}", page, cancellationToken);
